...
using namespace BongoEngine::Math;
...
void SimpleDraw::StaticsInitialize(uint32_t maxVertices)
{
	sSimpleDrawImpl = std::make_unique<SimpleDrawImpl>();
	sSimpleDrawImpl->Initialize(maxVertices);
}

void SimpleDraw::StaticsTerminate()
{
     if (sSimpleDrawImpl)
    {
...
{
    sSimpleDrawImpl->AddFace(v0, v1, v2, color);
}
